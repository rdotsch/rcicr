...
The second is derived, not named: it moves with each release, and a named one quietly stops
being the previous release. CI picks it the same way.
...
undecorated z-maps — have a comparable value from v1.1.0 on and none at all in v1.0.1.
...
Then trigger R-hub from the Actions tab against the release branch. Record the actual errors, warnings and NOTEs from each check, with its R version and platform. Do not carry the archived-package reinstatement NOTEs into a routine update. Report sandbox-only NOTEs as local results.

**Why results can be written into the commit they describe:** `cran-comments.md` is
`.Rbuildignore`d and verifiably absent from the tarball, so editing it does not change the package sources included in the tarball. Fresh builds can differ in packaging timestamps and metadata; byte identity is not assumed. It is not a package artifact at all — it is the text pasted into the submission
...
- **R-hub** — the stock v2 workflow, run from this repository's Actions tab. Because it is
  ...
  `gh run download <run-id> -D <dir>`, then read `*/rcicr.Rcheck/00check.log`. R-hub skips incoming feasibility and uses `--no-manual`. A green run does not establish either omitted check; obtain a complete manual check before submission.
...
**Ron submits personally.** CRAN emails the maintainer address to confirm, so complete that confirmation after uploading.
