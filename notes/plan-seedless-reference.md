...
3. **Report, with a message, when a stored reference cannot be regenerated from the file.** A file without `seed` (absent or `NULL`) whose stored reference is vouched (`vouchedReference()`: the `saved_noise` marker and a matching fingerprint) and has no `response_seed` cannot be regenerated from that file. It may have been drawn from the clock, or the `seed` may have been removed after a seeded build; `vouchedReference()` checks the marker and the norms, not what the file held when they were drawn, so the message claims only what can be checked. It is still used, so no number changes, but `computeInfoVal2IFC()` says, with `message()` rather than `warning()`, that it cannot be regenerated from this file, and how to replace it (the call in 1). A message, as in "A cached reference is trusted only on positive evidence" in `DECISIONS.md`, so that under `options(warn = 2)` the notice cannot stop the unchanged value being returned.
...
6. **Help pages.** `?generateReferenceDistribution2IFC`'s Reproducibility section promises the default reference depends only on the stimulus file, and both functions' `response_seed` documentation says `NULL` continues the saved stream. Each gains that this needs a saved stimulus seed, and what to pass without one. `save_rdata`'s help, which says to set it to `FALSE` whenever `response_seed` is set, gains the exception: on a file without a stimulus seed, storing the seeded reference is the point, since it becomes the file's reproducible reference. `?generateStimuli2IFC`'s `seed` gains, where the file is made, that `seed = NULL` leaves no seed to replay, so InfoVal references from that file need an explicit `response_seed`. `man/` is regenerated with roxygen.
