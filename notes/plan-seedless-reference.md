...
3. **Report, with a message, when a stored reference was drawn from the clock.** A file without `seed` (absent or `NULL`) whose stored reference has no `response_seed` got it from a clock-seeded draw. It is still used, so no number changes, but `computeInfoVal2IFC()` says, with `message()` rather than `warning()`, that it cannot be reproduced from the stimuli, and how to replace it (the call in 1). A message, as in "A cached reference is trusted only on positive evidence" in `DECISIONS.md`, so that under `options(warn = 2)` the notice cannot stop the unchanged value being returned.
4. **`NEWS.md`, under "Reproducibility impact"**, since forced and direct calls used to return numbers and now stop: the rows above; which versions (a file made with `seed = NULL`: every version measured, back to 1.0.1; a file without the field: 1.4.0 and 1.4.1); what happens now; how to get a reproducible reference; and that files with `seed` are unaffected.
...
- a seedless file carrying a stored reference without `response_seed` returns the same InfoVal as before, with the message, also under `options(warn = 2)`; one with a recorded `response_seed`, or a file with a real `seed`, gives no message.
