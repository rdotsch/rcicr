...
1. **One internal helper, `saveRdataSafely(names, file, envir)`, in `R/rdata.R`**, used by both call sites:
   ...
   - **if the staging file's name would exceed 255 bytes** (`nchar(basename(staging), type = "bytes")`, the usual file-name limit; the staging name is longer than the backup name, so it decides), save in place without a backup and warn that the name is too long to protect. The original's name must then be over about 225 bytes; rcicr's own names are about 40. That limit can be recognised before anything is written, so this fallback does not mask a full disk;
