...
- With the lock taken for `x_seed_1_time_T.Rdata`, a second `dir.create()` fails for `sub/../x_seed_1_time_T.Rdata` (with `sub` existing) and for `X_seed_1_time_T.Rdata`; the alias's lock resolves to the same physical directory.
...
- a held lock stops a second call before it writes anything, including one whose target is spelled differently (`label` differing only in case; a `sub/../` path); its error names the lock and says to delete it only once no call is writing to the folder;
