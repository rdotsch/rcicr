...
On 0.4.0 that sentence needs a caveat: `generateCI2IFC()` and `batchGenerateCI2IFC()` could not run at all there, stopping on every call with `must have 'max' > 'min'` because of a separate argument-misalignment defect (2016-10-28 to 2021-12-28, `development` branch only). They emit nothing rather than something wrong, so "cannot reach the defect" still holds, but `batchGenerateCI()` is the only per-participant route that actually worked on 0.4.0. See [`individual-ci-advisory-verification.md`](individual-ci-advisory-verification.md).
