...
"not affected" holds. It changes what to tell a 0.4.0-era reader: `batchGenerateCI()` is the
only per-participant route that both ran and named its output correctly there, and the advisory
now says so.
