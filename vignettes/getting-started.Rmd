...
- `batchGenerateCI()` and `batchGenerateCI2IFC()` compute one CI per participant or condition from a data frame. Follow them with `autoscale()` to put the whole batch on one scale, so the images can be compared by eye.
