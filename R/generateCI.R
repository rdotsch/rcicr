...
# `is.null(dim(mask))` keeps the sentinel test off one-cell matrices: without
# it, matrix(NA, 1, 1) is length 1 and NA, so it was read as "no mask" and
# discarded in silence rather than being rejected as non-binary.
