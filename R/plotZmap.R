...
#'
#' Takes a matrix of z-scores, such as the \code{zmap} that \code{\link{generateCI}} returns,
#' and optionally a background image matrix, and writes the z-map as a PNG.
#' Further arguments are passed to \code{graphics::image}; for example, \code{col} sets a
#' different colour palette. Versions up to and including 1.2.3 passed them to the raster
#' package's plot method instead. \code{col} works the same in both, but arguments specific to
#' that method are no longer understood.
#'
...
#'
#' This applies only to \code{plotZmap()}, the one function in the package that opens a graphics device. Every other PNG \code{rcicr} writes (stimuli, classification images, autoscaled classification images) comes straight from the pixel array via \code{png::writePNG()}.
#'
#' @export
#' @importFrom grDevices png dev.off terrain.colors
#' @importFrom graphics rasterImage par plot.new plot.window image axis
#' @param zmap Matrix of z-scores. Must have the same dimensions as \code{bgimage}.
#' @param bgimage Greyscale image matrix to draw behind the z-map, normally the base image or the final CI. Without it, only the z-map is drawn.
#' @param sigma Sigma of the smoothing applied to the CI to create the z-map, shown in the decoration.
#' @param threshold Threshold z-score (default: 3). Z-scores below it are not drawn on the z-map.
#' @param mask Optional mask: a binary matrix the size of \code{zmap} (0 or \code{FALSE} = masked, 1 or \code{TRUE} = kept), or the path to a black-and-white PNG image (black = masked, white = kept). This is the same convention as \code{generateCI()}'s \code{mask}, so one mask works for both. Earlier versions of this documentation described the matrix the wrong way round.
#' @param decoration Boolean: draw the z-map with margins, a caption (sigma, threshold) and a scale (default: \code{TRUE}).
#' @param targetpath Directory to save the z-map PNG to. Required, since writing that file is this function's purpose; there is no default. The directory is created if it does not exist; to just try the function out, use \code{tempdir()}.
#' @param filename Optional file name for the z-map PNG.
#' @param size Width and height of the PNG image, in pixels (default: 512).
