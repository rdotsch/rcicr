...
- **Stimulus sets generated by 1.0.1 and 1.1.0 are now covered by tests.** The suite carries
  ...
  classification image. Every other fixture is built by the current generator, so nothing
  before this could catch a field this version expects and an older one never saved. It runs on
  every platform with no network: the files are 46 KB each.
