...
- **Stimulus sets generated by 1.0.1 and 1.1.0 are now covered by tests.** The suite carries
  ...
  classification image. A 1.0.1 file predates `nscales` and `sigma`, so
  `generateReferenceDistribution2IFC()` has to assume them: the test asserts the null it builds
  is bit-identical to one from a file that states `nscales` outright, because a null built on
