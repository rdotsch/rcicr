...
- Because the checks are required, an infrastructure flake blocks a merge. **Re-run it from here** — `gh run rerun <id> --failed` works, and a workflow dispatch is permitted too: probe it with a bogus ref, which returns 422 (allowed, bad ref) rather than 403 (no permission).
...
| `DECISIONS-latent.md` | the same, for the gated latent-space module | 2400 |
...
- Parallelism is via base `parallel` + `doSNOW`/`foreach`, not newer alternatives (e.g. `future`) — match this pattern for new parallel code, and remember worker processes need `.packages='rcicr'` set on `foreach` calls. Tick progress bars from the parent with `.options.snow = progressOption(pb, cl)`; an in-body tick only updates a worker's private copy.
