...
> **State on 2026-08-08. CRAN reviewed 1.2.1 and asked for seven changes. 1.2.2 answered
> them, and 1.2.3 is what actually goes to CRAN.** The reply was a request, not a rejection
> on the merits: seven points, all mechanical apart from one. `cran-comments.md` opens with
> a point-by-point response to them.
>
> **1.2.2 was tagged and released but never submitted.** It answered CRAN's point 4
> ("some code lines in examples are commented out") by saying we could not find the
> commented line in the `.Rd` file they named — because there is none there. Sweeping
> *every* `.Rd` rather than the one named found it in `man/rcicr-package.Rd`, whose entire
> `\examples` section was `#simple examples will be added soon.`, left since 2016.
> Submitting 1.2.2 would have shipped the exact thing the reviewer asked us to stop doing,
> alongside a question inviting them to point at it a second time.
>
> **Why three sweeps walked past it:** that page was the package's only hand-maintained
> `.Rd`. Every other one is generated from roxygen, so the sweeps behind 1.2.2 — bare
> `T`/`F`, `\dontrun{}`, default write paths — all worked from `R/` and structurally could
> not see it. It is now generated too, and `grep -L "Generated by roxygen2" man/*.Rd`
> returns nothing, so the blind spot is closed rather than swept once.
>
> **1.2.3 is documentation only** — nothing the package computes differs from 1.2.2. It is
> **PR #170**, with every external check in and recorded in `cran-comments.md`: win-builder
> **1 NOTE** on both R-devel (r90366 ucrt) and R-release (4.6.1), the incoming-feasibility
> one and nothing else; R-hub **`Status: OK`, no notes**, on Linux (r90185), Windows
> (r90366 ucrt) and macOS (r90190). **What remains: merge #170, tag `v1.2.3`, publish the
> release, reopen `main` at `.9000` — then the maintainer submits to CRAN personally**,
> built from the tag rather than from `main`, which a submitted tarball must never be.
>
> **The one of the seven that mattered: no function may write to a default path.**
> `stimulus_path`, `targetpath` and `zmaptargetpath` lost their defaults (`./stimuli`,
> `./cis`, `./zmaps`) in 1.2.2 and are now required whenever the call actually writes. This
> is a **breaking change**, the first this package has made, and `NEWS.md` leads with it and
> the migration. It closes **item 24** for free. Numerically inert: the release gate reports
> 135 checks identical against v1.2.1, `max|d| = 0` everywhere.
>
...
> **every example now runs**, all of them well inside CRAN's five-second-per-example
> guideline; `plotZmap()` restores `par()`; the walkthrough vignette resets `par()` without
> `on.exit()`. **Do not restate the total runtime as a number** — it is machine-dependent,
> and two successive versions of `cran-comments.md` were caught claiming one the reviewer's
> own log would have contradicted.
>
...
v1.1.0.** Items 1, 20 and 21 are not code: item 20's checklist has now been run three times
and **1.2.3 is the version that goes to CRAN** (1.2.2 was tagged but superseded before
submission), so item 1 waits on the maintainer's resubmission and then on CRAN's reply, and
item 21 waits on the outcome. **Item 23 is fixed** — the `plotZmap()` mask is applied, under a "Behaviour change"
...
| 1 | CRAN archived | **1.2.1 was submitted 2026-07-29 and the review asked for seven changes. 1.2.2 answered them but was superseded before submission; 1.2.3 is what goes to CRAN**, and is PR #170. All external checks are in and recorded in `cran-comments.md`. What remains is merging, tagging, and the maintainer resubmitting personally from the tag's tarball — nothing else here is actionable until CRAN replies | M |
...
| 20 | CRAN resubmission checklist | **Run three times now — 1.2.1, 1.2.2 and 1.2.3**, which is what the checklist is for. At 1.2.3: `--as-cran` 0 errors / 0 warnings / 2 expected NOTEs, win-builder 1 NOTE on both R-devel and R-release, R-hub `Status: OK` on all three platforms. Keep it: every further round of CRAN review means running it again | M |
