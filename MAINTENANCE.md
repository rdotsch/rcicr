...
no required contexts and looks unconfigured. Adding a name to that ruleset is a repo-settings
write agents here cannot make, so a newly added workflow's check reports on PRs but blocks none
...
- **A step in `ubuntu-latest (release)`, not a new job.** A new job's check would report without
  ever blocking, for the ruleset reason above. It runs last so a failure cannot cost us the
  check results.
...
A hand-written file with a field-by-field comparator was **rejected**: every defect found in it
had the same shape — a field the comparison did not reach. Unordered URL membership let
`repository-code` and `url` swap; comparing authors by name left the *address* unchecked, on a
package CRAN archived for an undeliverable address. Such a comparator is a list of the fields
