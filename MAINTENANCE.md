...
language-agnostic hooks only. `styler` is deliberately left out: it
would reformat nearly every file in one sweep and destroy `git blame`.
`lintr` runs as `lint.yaml` instead of a pre-commit hook —
`object_usage_linter` needs the package *installed*, not just parsed,
which the language-agnostic hooks here never need and a pre-commit.ci
