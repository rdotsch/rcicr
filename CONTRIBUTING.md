...
- **Check any new function argument against the names saved in the `.Rdata`**, and any new
  ...
  recently a saved `sigma` capturing `generateCI()`'s z-map blur `sigma`. `DECISIONS.md` → "Numerics and the random number stream" has
  the details.
...
- **`ChangeLog`**: add a dated pointer entry deferring to `NEWS.md` — not a duplicate. See
  `DECISIONS.md` → "Releases and git" for why.
