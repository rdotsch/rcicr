...
that analysis: a classification image is only reproducible against the
version that computed it, and any release that changes numeric output
...
> The code was never the problem, and the maintainer address has since
> been updated. Returning to CRAN is being worked on; until then, GitHub
> is the only source. The last CRAN release was 0.3.4.1, which is
