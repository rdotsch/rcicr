...
Not finding it clears the analysis only if the output still sits as the
run left it. A directory that was renamed, folded into an archive, or
had its PNGs moved elsewhere will not answer to that name any more. The
filenames alone do not then settle it: an individual CI is
...
cleanly, permuting a set that had been correct.
...
IDs the way the original run did, which is the same condition the check
above depends on: the original `participants` object rather than a
