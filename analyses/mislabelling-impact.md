...
ordering_impact <- function(participants, true_correlation = 0.5,
  ...
  if (n_participants < 3) {
    return(list(share_correctly_paired = share_correctly_paired,
                affected = TRUE, note = "too few participants to estimate rates"))
  }
...
Pearson correlation on normally distributed data, so treat them as
indicative if the second stage was a condition comparison or a
regression. And `sort()` uses the session’s collation, which for
character identifiers may not be the collation the original analysis ran
under — lowercase ASCII is no guarantee, since some locales collate
letter pairs as units (Czech `ch` after `h`, Danish `aa` at the end of
the alphabet). Run it under the original locale where that matters.
