...
Do not read the individual rates too closely: the Monte Carlo SE at 0.05 is `r sprintf("%.4f", sqrt(0.05 * 0.95 / n_simulations))` at this many iterations, and each test has its own false-positive rate even under correct labels, the Welch *t* in design C being mildly conservative at N = 12. The statistic that carries the claim is the **paired** difference, both analyses seeing the same outcome vector within an iteration. It runs from `r sprintf("%+.3f", min(null_cells$difference))` to `r sprintf("%+.3f", max(null_cells$difference))`, each within `r sprintf("%.1f", max(null_cells$SEs_from_zero, na.rm = TRUE))` paired SEs of zero.
