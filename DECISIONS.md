...
### `CITATION.cff` is hand-written and checked, not generated by `cffr`
`cffr::cff_write()` would derive the file from `DESCRIPTION` and `inst/CITATION`, which is the
idiomatic R answer and removes the duplication at the source. It was rejected on two counts:
it adds a dev dependency with its own tree (`yaml`, `jsonlite`, `desc`) for one 18-line file,
and a generated file has to be regenerated by a *pinned* generator or the diff turns red on
formatting — the second version pin in this repo, after roxygen2's, for far less benefit.
