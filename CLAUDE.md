...
- **`tests/testthat/test-known-bugs.R` is expected to fail.** It is the executable form of the P0 items in `BACKLOG.md`: each test asserts the behaviour rcicr *should* have, so it fails until that bug is fixed and then goes green with no edit to the test. **The suite is not green on `main` by design** — treat `test-known-bugs.R` failures as the to-do list, and any *other* failure as a real regression. When the file is empty, the P0 backlog is done.
  - Do **not** "fix" these by asserting the current buggy output (`expect_error(<current wrong behaviour>)`). That passes while the bug exists and breaks when someone fixes it — it locks the bug in and punishes the fix. The suite previously did this for two bugs; it was reversed deliberately.
