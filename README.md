...
| `reference_norms_source` | What `reference_norms` was built from — `"saved_noise"` for a distribution computed from the file's own saved parameters and basis. Its absence on a file that never recorded `nscales` means the distribution predates that change and is regenerated once. Added after 1.3.0. |
